...
def normalize_features_global(
    ...
    all_e = np.concatenate([g.edge_attr.numpy() for g in graphs], axis=0)
...
def recompute_node_features(graph: Data) -> Data:
    ...
    pkt_count = graph.edge_attr[:, 0].numpy()
    total_bytes = graph.edge_attr[:, 1].numpy()
    n_nodes = graph.num_nodes or int(graph.edge_index.max()) + 1
