...
_DATA_ROOT = Path(
    r"C:\Users\Anusk\Downloads\SSDP_Flood_labels.csv\SSDP_Flood_labels.csv"
).parent
DEFAULT_PACKETS_CSV = str(_DATA_ROOT / "ssdp_packets_rich.csv")
DEFAULT_LABELS_CSV = str(_DATA_ROOT / "SSDP_Flood_labels.csv")
    ...
    for i in range(n_chunks):
        ...
        with open(fpath, "w", encoding="utf-8") as f:
            for _, row in chunk.iterrows():
                doc = {col: _safe_val(row[col]) for col in columns}
